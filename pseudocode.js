...
//generate the quote
  ...
  function generate(){ //function to create an array of quotes and authors
...
//randomQuote with Math.random iterating through quotes length.
		randomQuote = quotes[Math.floor(Math.random()*quotes.length)];
    quoteAuthor=randomQuote.split("@"); //split ramdomQuote var into saying and author at the @ symbol
		$('.saying').text(quoteAuthor[0]); // First element inserted into the .saying html element, while the secound element will be inserted into the .author HTML element.
    $('.author').text(quoteAuthor[1]); //
  }


$(".button").on( "click", function() {
    generate(); //on click of button generate quote and run function 
});
